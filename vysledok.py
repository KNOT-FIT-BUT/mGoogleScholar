...
def extendedSearch(AllWords, WithCorrectPhrase, LeastOneWord, WithoutWords, Occurence, Author, Venue, StartYear, EndYear):
	...
	base_url = ""
	base_url = sendGoogle_EXTENDED(
		AllWords, WithCorrectPhrase, LeastOneWord, WithoutWords, Occurence, Author, Venue, StartYear, EndYear)
	time.sleep(15)
	try:
		response = urllib2.urlopen(urllib2.Request(
			base_url, headers={"User-Agent": "Mozilla/5.0 Cheater/1.0"}))
...
def __sendGoogle_EXTENDED(AllWords, WithCorrectPhrase, LeastOneWord, WithoutWords, Occurence, Author, Venue, StartYear, EndYear):
    ...
    try:
        ...
        http_req = "http://scholar.google.cz/scholar?as_q=" + AllWords + "&as_epq=" +
                    WithCorrectPhrase + "&as_oq=" + LeastOneWord + "&as_eq=" + WithoutWords +
